...
export default function PartidosTable({ partidos, onDelete, onTogglePublicar }: Props) {
  const [publicarPartido, setPublicarPartido] = useState<Partido | null>(null);
  ...
  return (
    <>
            ...
            {partidos.map((p) => (
              <tr key={p.id} className="hover:bg-slate-50/50 transition-colors">
                ...
                <td className="px-4 py-3 text-right space-x-3 whitespace-nowrap">
                  ...
                  {p.activo ? (
                    <button
                      onClick={() => onTogglePublicar(p.id, false)}
      ...
      {publicarPartido && (
        <PublicarModal
          partido={publicarPartido}
          onClose={() => setPublicarPartido(null)}
          onConfirm={async (datos) => {
            await onTogglePublicar(publicarPartido.id, true, datos);
            setPublicarPartido(null);
          }}
        />
      )}
    </>
